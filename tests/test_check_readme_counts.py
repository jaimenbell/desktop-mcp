...
#   "tests-130 (128 passing, 2 skipped)-brightgreen" percent-encoded.
# ---------------------------------------------------------------------------
...
def test_parse_claimed_counts_matches_real_badge_phrasing():
    ...
    assert claim == check_readme_counts.Counts(total=130, passed=128, skipped=2)
...
<testsuites>
<testsuite name="pytest" errors="0" failures="0" skipped="2" tests="130" time="1.2">
</testsuite>
</testsuites>
"""
...
<testsuites>
<testsuite name="pytest" errors="0" failures="3" skipped="2" tests="130" time="1.2">
</testsuite>
</testsuites>
"""
...
def test_parse_actual_counts_from_junit(tmp_path):
    ...
    assert actual == check_readme_counts.Counts(total=130, passed=128, skipped=2)
...
def test_parse_actual_counts_subtracts_failures(tmp_path):
    ...
    actual = check_readme_counts.parse_actual_counts(junit_file)
    # 130 total - 2 skipped - 3 failures = 125 passed
    assert actual == check_readme_counts.Counts(total=130, passed=125, skipped=2)
...
def test_compare_match_passes():
    claimed = check_readme_counts.Counts(total=130, passed=128, skipped=2)
    actual = check_readme_counts.Counts(total=130, passed=128, skipped=2)
...
def test_compare_drift_fails():
    claimed = check_readme_counts.Counts(total=100, passed=98, skipped=2)
    actual = check_readme_counts.Counts(total=130, passed=128, skipped=2)
    ok, message = check_readme_counts.compare(claimed, actual)
    assert ok is False
...
def test_compare_missing_claim_fails():
    ok, message = check_readme_counts.compare(None, check_readme_counts.Counts(total=130, passed=128, skipped=2))
    assert ok is False
...
def test_main_exits_nonzero_on_drift(tmp_path):
    readme = tmp_path / "README.md"
    readme.write_text(
        "[![tests](https://img.shields.io/badge/tests-100%20%2898%20passing%2C%202%20skipped%29-brightgreen)](tests)\n",
        encoding="utf-8",
    )
    junit = tmp_path / "junit.xml"
    junit.write_text(_JUNIT_MATCH, encoding="utf-8")
